...
namespace OnionArcAndAll.Mapper.AutoMapper
{
            ...
            var config = new MapperConfiguration(cfg =>
            {
                foreach (var typePair in typePairs)
                {
                    if (ignore is not null)
                        cfg.CreateMap(typePair.SourceType, typePair.DestinationType).MaxDepth(depth).ForMember(ignore, opt => opt.Ignore()).ReverseMap();
                    else
